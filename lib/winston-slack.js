...
function Slack(config) {
    ...
    if (!config.token) throw new Error("Missing token in config");
...
Slack.prototype.log = function (level, msg, meta, callback) {
    ...
    var payload = {
        text: message.text,
        token: message.token || this.config.token,
    ...
    request.post(
        {
            uri: this.config.webhook_url || "https://slack.com/api/chat.postMessage",
            headers: {
